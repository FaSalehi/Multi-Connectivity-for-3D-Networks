...
BW_RB = 0.2e6;       % bandwidth of each resource block (RB) (Hz)
Nc = 4;              % number of RB allocated to each user (allocated bandwidth): {1,...,6}
...
D_th = [10e-3; 20e-3; 50e-3; 100e-3];      % delay threshold
n_smp = 1e3;       % number of channel generation
n_itr = 1e4;       % number of iterations
...
ii = 1;
for R = [10e3:10e3:300e3, 300e3:R_stp:R_max]
...
Net_avail_bu(1,ii) = sum((Etot_bu <= E_th))/(n_smp*n_itr);
Net_avail_uu(1,ii) = sum((Etot_Kuu(1,:) <= E_th),2)/(n_smp*n_itr);
Net_avail_BU1(1,ii) = sum((Etot_BU1 <= E_th))/(n_smp*n_itr);
Net_avail_BU2(1,ii) = sum((Etot_BU2 <= E_th))/(n_smp*n_itr);
Net_avail_BU3(1,ii) = sum((Etot_BU3 <= E_th))/(n_smp*n_itr);

Net_avail_hu(1,ii) = sum((Etot_hu <= E_th))/(n_smp*n_itr);
Net_avail_BH(1,ii) = sum((Etot_BH <= E_th))/(n_smp*n_itr);
Net_avail_BHU1(1,ii) = sum((Etot_BHU1 <= E_th))/(n_smp*n_itr);
Net_avail_BHU2(1,ii) = sum((Etot_BHU2 <= E_th))/(n_smp*n_itr);
Net_avail_BHU3(1,ii) = sum((Etot_BHU3 <= E_th))/(n_smp*n_itr);

Net_avail_su_S(1,ii) = sum((Etot_su_S <= E_th))/(n_smp*n_itr);
Net_avail_BS_S(1,ii) = sum((Etot_BS_S <= E_th))/(n_smp*n_itr);
Net_avail_BSU1_S(1,ii) = sum((Etot_BSU1_S <= E_th))/(n_smp*n_itr);
Net_avail_BSU2_S(1,ii) = sum((Etot_BSU2_S <= E_th))/(n_smp*n_itr);
Net_avail_BSU3_S(1,ii) = sum((Etot_BSU3_S <= E_th))/(n_smp*n_itr);

for t = 1:length(D_th)
Net_avail_bu(t+1,ii) = sum((Etot_bu <= E_th).*(Dtot_bu <= D_th(t)))/(n_smp*n_itr);
Net_avail_uu(t+1,ii) = sum((Etot_Kuu(1,:) <= E_th).*(Dtot_Kuu(1,:) <= D_th(t)),2)/(n_smp*n_itr);
Net_avail_BU1(t+1,ii) = sum((Etot_BU1 <= E_th).*(Dtot_BU1 <= D_th(t)))/(n_smp*n_itr);
Net_avail_BU2(t+1,ii) = sum((Etot_BU2 <= E_th).*(Dtot_BU2 <= D_th(t)))/(n_smp*n_itr);
Net_avail_BU3(t+1,ii) = sum((Etot_BU3 <= E_th).*(Dtot_BU3 <= D_th(t)))/(n_smp*n_itr);
...
% 
Net_avail_hu(t+1,ii) = sum((Etot_hu <= E_th).*(Dtot_hu <= D_th(t)))/(n_smp*n_itr);
Net_avail_BH(t+1,ii) = sum((Etot_BH <= E_th).*(Dtot_BH <= D_th(t)))/(n_smp*n_itr);
Net_avail_BHU1(t+1,ii) = sum((Etot_BHU1 <= E_th).*(Dtot_BHU1 <= D_th(t)))/(n_smp*n_itr);
Net_avail_BHU2(t+1,ii) = sum((Etot_BHU2 <= E_th).*(Dtot_BHU2 <= D_th(t)))/(n_smp*n_itr);
Net_avail_BHU3(t+1,ii) = sum((Etot_BHU3 <= E_th).*(Dtot_BHU3 <= D_th(t)))/(n_smp*n_itr);
...
% 
Net_avail_su_S(t+1,ii) = sum((Etot_su_S <= E_th).*(Dtot_su_S <= D_th(t)))/(n_smp*n_itr);
Net_avail_BS_S(t+1,ii) = sum((Etot_BS_S <= E_th).*(Dtot_BS_S <= D_th(t)))/(n_smp*n_itr);
Net_avail_BSU1_S(t+1,ii) = sum((Etot_BSU1_S <= E_th).*(Dtot_BSU1_S <= D_th(t)))/(n_smp*n_itr);
Net_avail_BSU2_S(t+1,ii) = sum((Etot_BSU2_S <= E_th).*(Dtot_BSU2_S <= D_th(t)))/(n_smp*n_itr);
Net_avail_BSU3_S(t+1,ii) = sum((Etot_BSU3_S <= E_th).*(Dtot_BSU3_S <= D_th(t)))/(n_smp*n_itr);
...
%========================== Error calculation =============================
for t = 1:length(D_th)
Err_bu(t,ii) = sum((Dtot_bu > D_th(t)))/(n_smp*n_itr);
Err_uu(t,ii) = sum((Dtot_Kuu(1,:) > D_th(t)))/(n_smp*n_itr);
Err_BU1(t,ii) = sum((Dtot_BU1 > D_th(t)))/(n_smp*n_itr);
Err_BU2(t,ii) = sum((Dtot_BU2 > D_th(t)))/(n_smp*n_itr);
Err_BU3(t,ii) = sum((Dtot_BU3 > D_th(t)))/(n_smp*n_itr);
...
Err_hu(t,ii) = sum((Dtot_hu > D_th(t)))/(n_smp*n_itr);
Err_BH(t,ii) = sum((Dtot_BH > D_th(t)))/(n_smp*n_itr);
Err_BHU1(t,ii) = sum((Dtot_BHU1 > D_th(t)))/(n_smp*n_itr);
Err_BHU2(t,ii) = sum((Dtot_BHU2 > D_th(t)))/(n_smp*n_itr);
Err_BHU3(t,ii) = sum((Dtot_BHU3 > D_th(t)))/(n_smp*n_itr);

Err_su_S(t,ii) = sum((Dtot_su_S > D_th(t)))/(n_smp*n_itr);
Err_BS_S(t,ii) = sum((Dtot_BS_S > D_th(t)))/(n_smp*n_itr);
Err_BSU1_S(t,ii) = sum((Dtot_BSU1_S > D_th(t)))/(n_smp*n_itr);
Err_BSU2_S(t,ii) = sum((Dtot_BSU2_S > D_th(t)))/(n_smp*n_itr);
Err_BSU3_S(t,ii) = sum((Dtot_BSU3_S > D_th(t)))/(n_smp*n_itr);
